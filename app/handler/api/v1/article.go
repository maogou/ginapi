...
func (a Article) Get(c *gin.Context) {
	param := service.ArticleRequest{ID: transform.StrTo(c.Param("id")).MustUInt32()}
...
func (a Article) List(c *gin.Context) {
	...
	svc := service.New(c.Request.Context())
	pager := app.Page{Page: app.GetPage(c), PageSize: app.GetPageSize(c)}
...
func (a Article) Update(c *gin.Context) {
	param := service.UpdateArticleRequest{ID: transform.StrTo(c.Param("id")).MustUInt32()}
	response := app.NewResponse(c)
	valid, errs := app.BindAndValid(c, &param)
	if !valid {
		global.Logger.Errorf(c, "app.BindAndValid errs: %v", errs)
...
func (a Article) Delete(c *gin.Context) {
	param := service.DeleteArticleRequest{ID: transform.StrTo(c.Param("id")).MustUInt32()}
	response := app.NewResponse(c)
	valid, errs := app.BindAndValid(c, &param)
	if !valid {
		global.Logger.Errorf(c, "app.BindAndValid errs: %v", errs)
